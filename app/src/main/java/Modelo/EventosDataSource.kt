...
class EventosDataSource(context: Context) {
    ...
    fun guardarEvento(description:String, dia:String){
        val values = ContentValues()
        values.put(ColumnEventos.DESCRIPCION_EVENTO,description)
        values.put(ColumnEventos.DIA_EVENTO,dia)
        database.insert(EVENTOS_TABLE_NAME, null, values)
    }

    fun modificarEvento(description: String,dia: String,IdEvento: Int){
        val values = ContentValues()
        values.put(ColumnEventos.DESCRIPCION_EVENTO,description)
        values.put(ColumnEventos.DIA_EVENTO,dia)
    ...
    companion object {
        ...
        val INSERT_EVENTOS_SCRIPT = (
                "insert into " + EVENTOS_TABLE_NAME + " values " +
                        "(null,'Inauguracion del festival del globo','Virenes 13')," +
